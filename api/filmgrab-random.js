...
export default async function handler(req, res) {
  ...
  try {
    // 1) Hit FilmGrab random and follow redirect to a film page
    ...
    // 2) Parse out a bunch of stills
    const { title, credits, images } = parseFilmGrab(html);
    if (!images.length) throw new Error("No stills found on the page.");

    // Pick one still at random
    const imageUrl = images[Math.floor(Math.random() * images.length)];

    // Build a same-origin proxy URL that your browser can load
    const base =
      process.env.BASE_URL ||
      (req.headers["x-forwarded-proto"] || "https") + "://" + req.headers.host;

    const proxiedImage = `${base}/api/filmgrab-proxy?src=${encodeURIComponent(
      imageUrl
    )}`;
    ...
    res.status(200).json({
      ...
      postUrl,
      imageUrl,     // original (may 404/hotlink-block on the client)
...
function largestFromSrcset(srcset) {
  ...
  for (const p of parts) {
    ...
    const url = space > -1 ? p.substring(0, space) : p;
...
function parseFilmGrab(html) {
  ...
  if (!title) {
    ...
    if (t2 && t2[1]) title = stripTags(t2[1]);
  }

  // First paragraph as "credits"
  let credits = null;
  ...
  // Collect images, prefer data-full-url -> largest srcset -> src
  const images = [];
  const seen = new Set();
  let idx = 0;
  while (true) {
    const start = content.indexOf("<img", idx);
    if (start === -1) break;
